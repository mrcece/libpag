...
namespace pag {
SimpleGlyph::SimpleGlyph(tgfx::GlyphID glyphId, std::string name, tgfx::Font font, bool isVertical)
...
std::shared_ptr<SimpleGlyph> SimpleGlyph::makeVerticalGlyph() const {
  auto glyph = std::shared_ptr<SimpleGlyph>(new SimpleGlyph());
  *glyph = *this;
  glyph->applyVertical();
  return glyph;
}

void SimpleGlyph::applyVertical() {
  if (_isVertical) {
    return;
  }
  _isVertical = true;
...
std::vector<std::shared_ptr<SimpleGlyph>> GetSimpleGlyphs(const TextDocument* textDocument,
  ...
  while (textStart < textStop) {
    ...
    auto glyph = std::make_shared<SimpleGlyph>(glyphId, name, textFont, isVertical);
    glyphMap[name] = glyph;
